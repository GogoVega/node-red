...
RED.editor = (function() {
    ...
    function validateNode(node) {
        ...
        if (node.type.indexOf("subflow:")===0) {
            ...
            if (node.credentials) {
                validationErrors = validationErrors.concat(validateCredentials(node, node._def.credentials, node.credentials));
            }
    ...
    function validateCredentials(node, definition, credentials) {
        let result = [];
        for (const prop in definition) {
            if (definition.hasOwnProperty(prop)) {
                const valid = validateCredential(node, definition, credentials, prop);
    ...
    function validateCredential(node, definition, credential, property) {
        let value;
        if (definition[property].type === "password") {
            if (credential[property] !== undefined) {
                value = credential[property];
            } else if (credential["has_" + property] === true) {
                value = "__PWRD__";
            }
        } else {
            value = credential[property];
        }
        return validateNodeProperty(node, definition, property, value);
    }
    ...
    function validateNodeProperty(node,definition,property,value) {
        ...
        if (valid && definition[property].type && RED.nodes.getType(definition[property].type) && !("validate" in definition[property])) {
            if (!value || value == "_ADD_") {
                valid = !(("required" in definition[property]) && definition[property].required);
                if (!valid && label) {
                    return RED._("validator.errors.missing-required-prop", {
                        prop: label
                    });
