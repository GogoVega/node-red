...
RED.editor = (function() {
    ...
    function validateCredentials(node, definition, credentials) {
        ...
        for (const prop in definition) {
            if (definition.hasOwnProperty(prop)) {
                var valid = validateCredential(node, definition, credentials, prop);
    ...
    function validateCredential(node, definition, credentials, property) {
        let value;
        if (definition[property].type == "password") {
            if (credentials[property] != undefined) {
                value = credentials[property];
            } else if (credentials["has_" + property] === true) {
                value = "__PWRD__";
            }
        } else {
            value = credentials[property];
        }
        return validateNodeProperty(node, definition, property, credentials[property]);
    }
