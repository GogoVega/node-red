...
RED.palette.editor = (function() {
    ...
    function _refreshNodeModule(module) {
        if (!nodeEntries.hasOwnProperty(module)) {
            ...
            if (moduleInfo.pending_version) {
                ...
                const entry = loadedIndex[module];
                if (entry.engines) {
                    let content = "";
                    for (const prop in entry.engines) {
                        if (entry.engines.hasOwnProperty(prop)) {
                            const engine = entry.engines[prop];
                            if (!engine.satisfied) {
                                content += RED._('palette.editor.badEngine', {
                                    engine: prop,
                    ...
                    if (content) {
                        nodeEntry.updateButton.addClass('disabled');
                        nodeEntry.updateButton.text(RED._('palette.editor.updateIncompatible'));
                        RED.popover.create({
                            target: nodeEntry.updateButton,
                            content: content,
                            trigger: "hover",
                            direction: "bottom",
                ...
                } else if (loadedIndex[module].updateAvailable) {
    ...
    function refreshCatalogues (done) {
        ...
        $.ajax({
            method: "GET",
            url: "/palette/catalogues",
            data: { reload: reloadCatalogues },
            dataType: "json",
            contentType: "application/json",
            // TODO: why auth is needed here?
            ...
            success: function (catalogues) {
                loadedCatalogs.push(...catalogues);
                // sort loadedCatalogs by e.index ascending
                loadedCatalogs.sort((a, b) => a.index - b.index);

                for (const catalogue of catalogues) {
                    loadedList = loadedList.concat(catalogue.modules);
                    catalogue.modules.forEach((m) => loadedIndex[m.id] = m);
                }
            ...
            error: function(_jqxhr, _textStatus, error) {
                console.warn("Error loading catalogues: ", error);
            },
        });

        reloadCatalogues = false;
    }
            ...
            addItem: function(container,i,object) {
                ...
                if (object.info) {
                    ...
                    if (nodeEntries.hasOwnProperty(entry.id)) {
                        ...
                        if (content) {
                            installButton.addClass('disabled');
                            installButton.text(RED._('palette.editor.incompatible'));
                            RED.popover.create({
                                target: installButton,
                                content: content,
                                trigger: "hover",
        ...
        pendingRefreshTimeout = setTimeout(() => {
            ...
            for (const module of Object.keys(nodeEntries)) {
                if (loadedIndex.hasOwnProperty(module)) {
                    ...
                    if (loadedIndex[module].updateAvailable) {
