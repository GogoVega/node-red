...
function checkUpdate(entry, module) {
    delete entry.updateAvailable;
    // Update is allowed for this module
    if (updateAllowed && checkModuleAllowed(entry.id, entry.version, updateAllowList, updateDenyList)) {
...
async function refreshCatalogues(reload) {
    ...
    for (const catalogue of loadedCatalogs) {
        catalogue.modules = catalogue.modules.filter(function (m) {
            const moduleInstalled = !!moduleList[m.id];
            const moduleInstallAllowed = checkModuleAllowed(m.id, m.version, installAllowList, installDenyList);

            if (moduleInstalled || (installAllowed && moduleInstallAllowed)) {
...
function getSettings() {
    const settingsAllowList = runtime.settings.get("externalModules.palette.allowList")
    const settingsDenyList = runtime.settings.get("externalModules.palette.denyList")
    const settingsUpdateAllowList = runtime.settings.get("externalModules.palette.allowUpdateList")
    const settingsUpdateDenyList = runtime.settings.get("externalModules.palette.denyUpdateList")
    ...
    installAllowed = runtime.settings.get("externalModules.palette.allowInstall") ?? true;
    installAllowList = parseModuleList(installAllowList);
    installDenyList = parseModuleList(installDenyList);

    updateAllowed = runtime.settings.get("externalModules.palette.allowUpdate") ?? true;
    updateAllowList = parseModuleList(updateAllowList);
    updateDenyList = parseModuleList(updateDenyList);

    catalogues = runtime.settings.get("editorTheme.palette.catalogues") || ["https://catalogue.nodered.org/catalogue.json"];
}
